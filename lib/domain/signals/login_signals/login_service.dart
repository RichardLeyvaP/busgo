...
Future<void> updatePassword(int idUser,String currentPassword, String newPassword) async {
  isLoadingLG.value = true; // Indicamos que está cargando


 try {
  final result = await authRepository.updatePasswordRepo(idUser, currentPassword, newPassword);
  isLoadingLG.value = false; // Detenemos el loading
  ...
  if (result.containsKey('body')) {
    ...
    if (body.containsKey('details')) {
      passwordMessageLG.value = body['details'][0]; // Mensaje de la lista 'details'
